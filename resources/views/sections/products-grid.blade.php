<div x-data class="relative">
    ...
    <div class="flex gap-10 overflow-x-scroll snap-x" x-ref="carossel">

        @foreach ($products as $product)
            <a href="{{ route('product.view', [$product->brand->slug, $product->slug]) }}"
                class="shrink-0 flex m-auto flex-col justify-between snap-start border rounded-md w-[264px] h-[360px] hover:shadow-2xl transition-all duration-500">
    ...
    <div class="absolute left-0 inset-y-0 m-auto h-fit p-1 bg-primary rounded-full"
    ...
    <div class="absolute right-0 inset-y-0 m-auto h-fit p-1 bg-primary rounded-full"
        @click="$refs.carossel.scrollBy({left:200, behavior: 'smooth'})">
