    ...
    <ul class="absolute bg-white rounded-xl w-full shadow-xl z-20" x-show="$wire.search != '' "
        ...
        @foreach ($products as $product)
            <li class="hover:bg-gray-200 py-2 border-b">
                <a href="{{ route('product.details', [$product->category->slug, $product->slug]) }}" class="grid grid-cols-4 items-center">

                    <img src="{{ asset('storage/' . $product->images[0]) }}" alt="{{ $product->name }}" class="p-2">
